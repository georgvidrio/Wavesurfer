...
%         end  % function
        ...
        function checkSICommand_(self)
            ...
            if exist(self.wsCmdFullPath_,'file')
                str = readAllLines_(self.wsCmdFullPath_);
        ...
        function acknowledgeSICommand_(self,success)
            ...
            [fid,fopenErrorMessage]=fopen(self.wsRspnsFullPath_,'w');
            assert(fid>=0,'WavesurferModel:UnableToOpenYokingFile', ...
        ...
        function [areFilesGone,errorMessage]=ensureYokingFilesAreGone_(self) %#ok<MANU>
            ...
            if exist(self.siCmdFullPath_,'file') ,
                ws.deleteFileWithoutWarning(self.siCmdFullPath_);
                if exist(self.siCmdFullPath_,'file') , 
            ...
            if exist(self.siRspnsFullPath_,'file') ,
                ws.deleteFileWithoutWarning(self.siRspnsFullPath_);
                if exist(self.siRspnsFullPath_,'file') , 
            ...
            % Ensure the response file is gone
            if exist(self.wsRspnsFullPath_,'file') ,
                ws.deleteFileWithoutWarning(self.wsRspnsFullPath_);
                if exist(self.wsRspnsFullPath_,'file') , 
                    isWsResponseFileGone=false;
                    if isempty(errorMessage1) 
                        errorMessage='Unable to delete pre-existing WaveSurferge response file';
                    else
                        errorMessage='Unable to delete pre-existing WaveSurfer yoking files';
                    end
                else
            ...
            % Ensure the command file is gone
            if exist(self.wsCmdFullPath_,'file') ,
                ws.deleteFileWithoutWarning(self.wsCmdFullPath_);
                if exist(self.wsCmdFullPath_,'file') , 
            ...
            areFilesGone=isCommandFileGone&&isResponseFileGone&&isWsResponseFileGone&&isWsCommandFileGone;
        end  % function
...
%             end
        ...
        function sendCommandToScanImage_(self,cmd)
            ...
            [fid,fopenErrorMessage]=fopen(self.siCmdFullPath_,'wt');
            if fid<0
        ...
        function [isScanImageReady,errorMessage]=waitForScanImageResponse_(self) %#ok<MANU>
            ...
            for iCheck=1:nChecks ,
                ...
                doesReponseFileExist=exist(self.siRspnsFullPath_,'file');
                if doesReponseFileExist ,
                    [fid,fopenErrorMessage]=fopen(self.siRspnsFullPath_,'rt');
                    if fid>=0 ,                        
                        response=fscanf(fid,'%s',1);
                        fclose(fid);
                        if isequal(response,'OK') ,
                            ws.deleteFileWithoutWarning(self.siRspnsFullPath_);  % We read it, so delete it now
            ...
            if exist(self.siRspnsFullPath_,'file') ,
                ws.deleteFileWithoutWarning(self.siRspnsFullPath_);  % If it exists, it's now a response to an old command
            end
            isScanImageReady=false;
